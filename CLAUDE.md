...
本项目的设计文档（designs/rag-service-design.md）是全流程唯一规格真源。历史上出现过 10 次"设计承诺→实现缺失"问题，其中 4 次是我（Claude）在写任务 brief 时**为了推进度悄悄缩小范围**（静默降级）。以下是强制规则：
...
10. **数据文件清理三重防线**（2026-08-13 事故：smoke 前 `rm -f workspace/cache.db` 把评估写好的 eval_history + per_qa 明细全部误删，before/after 对比依据丢失）：
    ...
    - 删除数据文件前先备份（`cp workspace/cache.db /tmp/backup-<ts>.db`）
